...
#     f, _ = write_file("imagenette_results", "ImageNette", "VGG-16", num_epochs, subset_size)
...
def execute():
    ...
    if torch.cuda.is_available():
        ...
        test_model(model, gpu_device, description="Testing")
        plot_loss_per_epoch(num_epochs, train_losses, "Training Loss Per Epoch (ImageNette VGG-16)", f"{PATH}/loss_per_epoch")
    else:
        print("GPU not available.")
...
if __name__ == "__main__":
    ...
    subset_size = args.subset
    train_loader, test_loader = setup()
    f, PATH = write_file("imagenette_results", "ImageNette", "VGG-16", num_epochs, subset_size)
    f.write("Using VGG Model on ImageNette\n")
    execute()
