...
@timer
def train_model(device, num_epochs, train_loader, description="Training"):
    model = SimpleResNet().to(device)
    criterion = nn.CrossEntropyLoss()
    optimizer = optim.Adam(model.parameters(), lr=0.001)
...
def execute():
    if torch.cuda.is_available():
        print("Training on GPU...")
        gpu_device = torch.device("cuda")
        model, train_losses = train_model(gpu_device, num_epochs, train_loader, description="Training")
        test_model(model, gpu_device, description="Testing")
        plot_loss_per_epoch(num_epochs, train_losses, "Training Loss Per Epoch (ImageNet Mini - Simple ResNet)", f"{PATH}/loss_per_epoch")
    else:
        print("GPU not available.")
...
if __name__ == "__main__":
    ...
    subset_size = args.subset
    train_loader, test_loader = setup()
    f, PATH = write_file("max_testing", "ImageNet", "Simple ResNet", num_epochs, subset_size)
    f.write("Using Simple ResNet Model\n")
    execute()